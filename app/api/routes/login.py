...
async def continue_google_token(
    ...
    user_oauth_id = f'google-{user_info.get('resourceName').split('/')[1]}'  # "people/{accountID}"
    user_birthyear = str(user_info.get('birthdays')[0].get('date').get('year'))  # YYYY

    try:
        user_gender = user_info.get('genders')[1].get('value')  # str: female or male
    except TypeError:  # 해당 항목이 없을 수 있음(Rather not say)
        user_gender = None
    ...
    if not user:  # 없으면 회원가입 시키고 토큰 발급
        user_create = UserCreate.model_validate({
            "email":        None,
            "birthyear":    user_birthyear,
            "sex":          True if user_gender == 'male' else False if user_gender == 'female' else None,
            "oauth_id":     user_oauth_id,
            "password":     None,
        })
        user = crud.create_user(session=session, user_create=user_create)
